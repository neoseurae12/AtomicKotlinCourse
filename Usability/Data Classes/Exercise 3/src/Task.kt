...
fun createAuthorToBooksMap(books: List<Book>): Map<Author, List<Book>> {
  val authorToBooksMap = mutableMapOf<Author, MutableList<Book>>()
  for (book in books) {
    for (author in book.authors) {
      if (!authorToBooksMap.containsKey(author)) {
        authorToBooksMap[author] = mutableListOf()
      }
      authorToBooksMap.getValue(author) += book   // 주의

      /** 안 되는 코드의 목록
       * 1) authorToBooksMap += author to book
       * 2) authorToBooksMap[author] += book
       * */
    }
  }
